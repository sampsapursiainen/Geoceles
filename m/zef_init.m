%Copyright © 2018, Sampsa Pursiainen  
          zef.inv_init_guess_mode = [1];
          zef.inv_eit_noise = 0;
          zef.inv_bg_data = [];
          zef.inv_roi_perturbation = 200;
          zef.current_pattern = [];
          zef.background_data = [];
          zef.inv_multires_n_decompositions = [10];
          zef.inv_multires_dec = []; 
          zef.inv_multires_ind = []; 
          zef.inv_multires_count = [];
          zef.inv_multires_n_levels = [3];
          zef.inv_multires_sparsity = 4;
          zef.inv_multires_n_iter = [10 10 10];
          zef.h_rec_source = [];
          zef.h_synth_source = [];
          zef.h_roi_sphere = [];
          zef.inv_rec_source = [0 0 0 1 0 0 0 3 1];
          zef.inv_synth_source = [0 0 0 1 0 0 10 0 3 1];
          zef.inv_roi_mode = 1;
          zef.inv_roi_threshold = 0.5;
          zef.inv_roi_sphere = [0 0 0 50];
          zef.inv_n_sampler = 10000; 
          zef.inv_n_burn_in = 1000; 
          zef.reconstruction_type = 6;
          zef.h_colorbar   = [];
          zef.location_unit= 1;
              zef.elevation= 0;
                zef.azimuth= 0;
           zef.axes_visible= 0;
              zef.n_sources= 500;
               zef.sc_sigma= 1000;
               zef.sk_sigma= 1000;
                zef.c_sigma= 1000;
                zef.g_sigma= 1000;
                zef.w_sigma= 1000;
               zef.d1_sigma= 1000;
               zef.d2_sigma= 1000;
               zef.d3_sigma= 1000;
               zef.d4_sigma= 1000;
        zef.mesh_resolution= 5;
      zef.attach_electrodes= 0;
              zef.s_visible= 1;
               zef.s_points= [];
           zef.s_directions= [];
           zef.s_directions_g = [];
           zef.d1_triangles=[];
              zef.d1_points=[];
            zef.d2_triangles= [];
               zef.d2_points= [];
            zef.d3_triangles= [];
               zef.d3_points= [];
            zef.d4_triangles= [];
               zef.d4_points= [];  
            zef.w_triangles= [];
               zef.w_points= [];
 zef.source_direction_mode = 3;
             zef.sc_visible= 1;
             zef.sk_visible= 1;
              zef.c_visible= 1;
              zef.g_visible= 1;
              zef.w_visible= 1;
             zef.d1_visible= 1;
               zef.d2_visible=1;
               zef.d3_visible=1;
               zef.d4_visible=1;
               zef.d1_scaling=1.0;
               zef.d2_scaling=1.0;
               zef.d3_scaling=1.0;
               zef.d4_visible=1;
             zef.sc_scaling= 1.0;
        zef.d1_zx_rotation= 0;
         zef.d1_yz_rotation= 0;
         zef.d1_xy_rotation= 0;
        zef.d1_z_correction= 0;
        zef.d1_y_correction= 0;
        zef.d1_x_correction= 0;
         zef.d2_zx_rotation= 0;
         zef.d2_yz_rotation= 0;
         zef.d2_xy_rotation= 0;
        zef.d2_z_correction= 0;
        zef.d2_y_correction= 0;
        zef.d2_x_correction= 0;
        zef.d3_zx_rotation= 0;
         zef.d3_yz_rotation= 0;
         zef.d3_xy_rotation= 0;
        zef.d3_z_correction= 0;
        zef.d3_y_correction= 0;
        zef.d3_x_correction= 0;
         zef.d4_zx_rotation= 0;
         zef.d4_yz_rotation= 0;
         zef.d4_xy_rotation= 0;
        zef.d4_z_correction= 0;
        zef.d4_y_correction= 0;
        zef.d4_x_correction= 0; 
         zef.sc_zx_rotation= 0;
         zef.sc_yz_rotation= 0;
         zef.sc_xy_rotation= 0;
        zef.sc_z_correction= 0;
        zef.sc_y_correction= 0;
        zef.sc_x_correction= 0;
             zef.sk_scaling= 1.0;
         zef.sk_zx_rotation= 0;
         zef.sk_yz_rotation= 0;
         zef.sk_xy_rotation= 0;
        zef.sk_z_correction= 0;
        zef.sk_y_correction= 0;
        zef.sk_x_correction= 0;
              zef.c_scaling= 1.0;
          zef.c_zx_rotation= 0;
          zef.c_yz_rotation= 0;
          zef.c_xy_rotation= 0;
         zef.c_z_correction= 0;
         zef.c_y_correction= 0;
         zef.c_x_correction= 0;
              zef.g_scaling= 1.0;
          zef.g_zx_rotation= 0;
          zef.g_yz_rotation= 0;
          zef.g_xy_rotation= 0;
         zef.g_z_correction= 0;
         zef.g_y_correction= 0;
         zef.g_x_correction= 0;
              zef.w_scaling= 1.0;
          zef.w_zx_rotation= 0;
          zef.w_yz_rotation= 0;
          zef.w_xy_rotation= 0;
         zef.w_z_correction= 0;
         zef.w_y_correction= 0;
         zef.w_x_correction= 0;
              zef.s_scaling= 1.0;
          zef.s_zx_rotation= 0;
          zef.s_yz_rotation= 0;
          zef.s_xy_rotation= 0;
         zef.s_z_correction= 0;
         zef.s_y_correction= 0;
         zef.s_x_correction= 0;
         zef.imaging_method= 4;
                  zef.d1_on= 0;
                  zef.d2_on= 0;
                  zef.d3_on= 0; 
                  zef.d4_on= 0;
                  zef.sc_on= 0;
                  zef.sk_on= 0;
                   zef.c_on= 0;
                   zef.g_on= 0;
           zef.sc_triangles= [];
              zef.sc_points= [];
           zef.sk_triangles= [];
              zef.sk_points= [];
            zef.c_triangles= [];
               zef.c_points= [];
            zef.g_triangles= [];
               zef.g_points= [];
               zef.sensors = [];
               zef.reuna_p = cell(0);
               zef.reuna_t = cell(0);
               zef.nodes   = [];
                zef.tetra  = [];
                   zef.w_on= 0;
                 zef.cam_va= 10;
         zef.preconditioner= 1;
      zef.solver_tolerance = 1e-8;
zef.preconditioner_tolerance= 0.001;
               zef.sigma_ind=[];
               zef.sigma=[];
               zef.sigma_vec=[];
               zef.sigma_mod=0;
zef.sensors_attached_volume = [];
      zef.surface_triangles = [];
      zef.n_sources_mod     = 0; 
      zef.n_sources_old     = 50000;
 zef.location_unit_current = 1;
                      zef.L = [];
       zef.source_positions = [];
       zef.source_directions = [];
              zef.brain_ind = [];
             zef.source_ind = [];
             zef.w_priority = 5;
             zef.g_priority = 4;
             zef.c_priority = 3;
            zef.sk_priority = 2;
            zef.sc_priority = 1;
            zef.d1_priority = 9;
            zef.d2_priority = 8;
            zef.d3_priority = 7;
            zef.d4_priority = 6;
            zef.cp_on       = 0;
            zef.cp_a        = 1;
            zef.cp_b        = 0;
            zef.cp_c        = 0;
            zef.cp_d        = 0;
       zef.meshing_accuracy = 0.1;
       zef.on_screen        = 0;
       zef.import_mode      = 0;
       zef.s_color = [0 0 1];
       zef.d1_color = [0.55 0.55 1];
       zef.d2_color =  1.05*[0.45 0.35 0];
       zef.d3_color = [0.8 0.3 0];
       zef.d4_color = [0.8 0.8 0.3];
       zef.w_color = [0.55 0.55 0.55];
       zef.g_color = [0.35 0.35 0.35];
       zef.c_color = 0.6*[0.7 0.8 0.7];
       zef.sk_color = [0.7 0.7 0.6];
       zef.sc_color = 0.7*[0.83 0.67 0.49];
       zef.mesh_smoothing_on = 0;
       zef.wm_sources = 0;
       zef.wm_sources_old = -1;
       zef.prism_layers = 0;
       zef.n_prism_layers = 2;
       zef.prism_size = 0.01;
       zef.prisms = [];
       zef.sigma_prisms = [];
       zef.refinement_on = 0;
       zef.smoothing_strength = 1;
       zef.smoothing_steps_surf = 15;
       zef.smoothing_steps_vol = 1;
       zef.refinement_type = 1;
       zef.surface_sources = 1; 
       zef.visualization_type = 1;
       zef.source_interpolation_on = 0;
       zef.measurements = [];
       zef.reconstruction = [];
       zef.inv_hyperprior = [1]; 
       zef.inv_beta = [1.5];
       zef.inv_theta0 = [0.001];
       zef.inv_likelihood_std = 0.01;
       zef.inv_n_map_iterations = [25];
       zef.inv_pcg_tol = [1e-6];
       zef.inv_sampling_frequency = [1025];
       zef.inv_low_cut_frequency = [7];
       zef.inv_high_cut_frequency = [9];
       zef.inv_data_segment = [1];
       zef.source_interpolation_ind = [];
       zef.cp2_on = 0;
       zef.cp2_a = 1;
       zef.cp2_b = 0;
       zef.cp2_c = 0;
       zef.cp2_d = 0;
       zef.cp3_on = 0;
       zef.cp3_a = 1;
       zef.cp3_b = 0;
       zef.cp3_c = 0;
       zef.cp3_d = 0;
       zef.inv_dynamic_range = 1e6; 
       zef.inv_scale = 2;
       zef.inv_colormap = 11;
       zef.layer_transparency = 1;
       zef.meshing_threshold = 0.5;
       zef.clear_axes1 = 1;
       zef.normalize_data = 1;
       zef.cp_mode = 1;
       zef.inv_time_1 = 0;
       zef.inv_time_2 = 0;
       zef.inv_time_3 = 0;
       zef.number_of_frames = 1;
       zef.frame_start = 0;
       zef.frame_stop = 0;
       zef.frame_step = 1;
       zef.orbit_1 = 0;
       zef.orbit_2 = 0;
       zef.non_source_ind = [];
       zef.source_model = 2;
       zef.use_depth_electrodes = 0;
